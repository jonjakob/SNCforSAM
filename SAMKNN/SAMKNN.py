...
class SAMKNN(BaseClassifier):
	...
	sizeDownTec: string, optional (default='kmeans')
		Method used to size down the LTM when maxSize is reached.
		It must either be 'kmeans' or 'snc' (Stochastic Neighbour Compression)
    """
    def __init__(self, n_neighbors=5, knnWeights='distance', maxSize=5000, LTMSizeProportion = 0.4, minSTMSize=50, STMSizeAdaption='maxACCApprox', useLTM=True, listener=[], sizeDownTec='kmeans'):
        ...
        if sizeDownTec == 'kmeans':
            self.getSizeDownFct = self.clusterDown
        elif sizeDownTec == 'snc':
            self.getSizeDownFct = SNC.snc
    ...
    def clusterDown(self, samples, labels):
        ...
        for label in uniqueLabels:
            ...
            newLength = max((tmpSamples.shape[0]/100)*50, 1)
    ...
    def sizeCheckSTMLTM(self):
        ...
        if len(self._STMLabels) + len(self._LTMLabels) > self.maxSTMSize + self.maxLTMSize:
            if len(self._LTMLabels) > self.maxLTMSize:
                self._LTMSamples, self._LTMLabels = self.getSizeDownFct(self._LTMSamples, self._LTMLabels)
            else:
                if len(self._STMLabels) + len(self._LTMLabels) > self.maxSTMSize + self.maxLTMSize:
                    STMShortened = True
